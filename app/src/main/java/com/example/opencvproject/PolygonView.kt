...
class PolygonView : FrameLayout {
    ...
    protected var polygonView: PolygonView? = null
...
//    }
    ...
    fun getOrderedPoints(points: List<PointF>?): Map<Int?, PointF?> {
        ...
        val orderedPoints: MutableMap<Int?, PointF?> = HashMap()
        if (points != null) {
            // Sort the points based on their position relative to the center point
            val sortedPoints = points.sortedWith(compareBy({ it.y }, { it.x }))

            for ((index, pointF) in sortedPoints.withIndex()) {
                orderedPoints[index] = pointF
            }
        }
    ...
    inner class MidPointTouchListenerImpl(
