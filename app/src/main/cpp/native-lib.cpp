...
vector<Point> getPoints(Mat image)
{
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
        ...
        // Check if the contour is a quadrilateral with specific aspect ratio or size constraints
        if (approx.size() == 4 && isContourConvex(Mat(approx)))
        {
            double aspectRatio = boundingRect(approx).width / static_cast<double>(boundingRect(approx).height);
            double area = contourArea(approx);

            // Adjust the conditions below as per the specific requirements of your document shape
            if (aspectRatio > 0.5 && aspectRatio < 2.0 && area > 1000)
            {
                points = approx;
                break;  // Stop processing after finding the first valid document contour
            }
        }else{
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Skipped Contour %f",i);
        }
    }
...
Java_com_example_opencvproject_MainActivity_getScannedBitmap
...
extern "C" JNIEXPORT jobject JNICALL
Java_com_example_opencvproject_MainActivity_getMagicColorBitmap
...
extern "C" JNIEXPORT jobject JNICALL
Java_com_example_opencvproject_MainActivity_getBWBitmap
        (JNIEnv *env, jobject thiz,jobject bitmap)
{
...
extern "C" JNIEXPORT jobject JNICALL
Java_com_example_opencvproject_MainActivity_getGrayBitmap
        (JNIEnv *env, jobject thiz,jobject bitmap)
{
...
Java_com_example_opencvproject_MainActivity_getPoints(JNIEnv *env, jobject thiz, jobject bitmap)
{
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Scanning getPoints");
    int ret;
    AndroidBitmapInfo info;
    void* pixels = 0;
    ...
    std::vector<cv::Point> img_pts = getPoints(mbgra);

    jfloatArray jArray = env->NewFloatArray(img_pts.size() * 2);

    if (jArray != NULL) {
        jfloat* ptr = env->GetFloatArrayElements(jArray, NULL);

        for (size_t i = 0, j = i + img_pts.size(); j < img_pts.size() * 2; i++, j++) {
            ptr[i] = img_pts[i].x;
            ptr[j] = img_pts[i].y;
        }
...
//extern "C" JNIEXPORT jfloatArray JNICALL
//Java_com_example_opencvproject_MainActivity_getPoints
//        (JNIEnv *env, jobject thiz,jobject bitmap)
//{
...
//Java_com_example_myapp_DocumentProcessor_processDocument(JNIEnv *env, jobject, jlong inputAddr, jlong outputAddr) {
//    // Convert input and output addresses to OpenCV Mat objects
...
//    Mat thresholdMat;
//    adaptiveThreshold(grayMat, thresholdMat, 255.0, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 15, 15);
//
//    // Find contours in the binary image
//    std::vector<std::vector<Point>> contours;
//    std::vector<Vec4i> hierarchy;
//    findContours(thresholdMat, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
//
...
//    }
//
//    // Draw a bounding rectangle around the document contour
//    if (maxContourIdx >= 0) {
//      //  Rect boundingRect = boundingRect(contours[maxContourIdx]);
//      //  rectangle(inputMat, Point(boundingRect.x, boundingRect.y), Point(boundingRect.x + boundingRect.width, boundingRect.y + boundingRect.height), Scalar(0, 255, 0), 3);
//    }
//
//    // Copy the inputMat to the outputMat
